...
class IFMR_Spera15(IFMR):
    ...
    def generate_death_mass(self, mass_array, metallicity_array):
        ...
        * BH: typecode = 103
        ...
        rem_mass_array[very_low_metal_low_mass_idx] = self.M_rem_very_low_metal_low_mass(
            Z_array[very_low_metal_low_mass_idx], core_mass[very_low_metal_low_mass_idx])
        ...
        rem_mass_array[very_low_metal_med_mass_idx] = self.M_rem_very_low_metal_med_mass(
            Z_array[very_low_metal_med_mass_idx], core_mass[very_low_metal_med_mass_idx])

        # remnant masses of stars with Z < 5.0e-4 and Mco > 10.0
        very_low_metal_high_mass_idx = np.where((Z_array < 5.0e-4) & (core_mass > 10.0))
        rem_mass_array[very_low_metal_high_mass_idx] = self.M_rem_very_low_metal_high_mass(
            Z_array[very_low_metal_high_mass_idx], core_mass[very_low_metal_high_mass_idx])

        #### low metallicity 5.0e-4 <= Z < 1.0e-3

        # remnant masses of stars with 5.0e-4 <= Z < 1.0e-3 and Mco < 5.0
        low_metal_low_mass_idx = np.where(
            (Z_array >= 5.0e-4) & (Z_array < 1.0e-3) & (core_mass < 5.0) & (core_mass >= 0))
        rem_mass_array[low_metal_low_mass_idx] = self.M_rem_low_metal_low_mass(Z_array[low_metal_low_mass_idx],
                                                                               core_mass[low_metal_low_mass_idx])

        # remnant masses of stars with 5.0e-4 <= Z < 1.0e-3 and 5.0 <= Mco <= 10.0
        low_metal_med_mass_idx = np.where(
            (Z_array >= 5.0e-4) & (Z_array < 1.0e-3) & (core_mass >= 5.0) & (core_mass <= 10.0))
        rem_mass_array[low_metal_med_mass_idx] = self.M_rem_low_metal_med_mass(Z_array[low_metal_med_mass_idx],
                                                                               core_mass[low_metal_med_mass_idx])
        ...
        rem_mass_array[low_metal_high_mass_idx] = self.M_rem_low_metal_high_mass(Z_array[low_metal_high_mass_idx],
                                                                                 core_mass[low_metal_high_mass_idx])

        #### medium metallicity 1.0e-3 <= Z <= 4.0e-3

        # remnant masses of stars with 1.0e-3 <= Z <= 4.0e-3 and Mco < 5.0
        med_metal_low_mass_idx = np.where(
            (Z_array >= 1.0e-3) & (Z_array <= 4.0e-3) & (core_mass < 5.0) & (core_mass >= 0))
        rem_mass_array[med_metal_low_mass_idx] = self.M_rem_med_metal_low_mass(Z_array[med_metal_low_mass_idx],
                                                                               core_mass[med_metal_low_mass_idx])

        # remnant masses of stars with 1.0e-3 <= Z <= 4.0e-3 and 5.0 <= Mco <= 10.0
        med_metal_med_mass_idx = np.where(
            (Z_array >= 1.0e-3) & (Z_array <= 4.0e-3) & (core_mass >= 5.0) & (core_mass <= 10.0))
        rem_mass_array[med_metal_med_mass_idx] = self.M_rem_med_metal_med_mass(Z_array[med_metal_med_mass_idx],
                                                                               core_mass[med_metal_med_mass_idx])
        ...
        rem_mass_array[med_metal_high_mass_idx_1] = self.M_rem_med_metal_high_mass_1(Z_array[med_metal_high_mass_idx_1],
                                                                                     core_mass[
                                                                                         med_metal_high_mass_idx_1])
        ...
        rem_mass_array[med_metal_high_mass_idx_2] = self.M_rem_med_metal_high_mass_2(Z_array[med_metal_high_mass_idx_2],
                                                                                     core_mass[
                                                                                         med_metal_high_mass_idx_2])
        ...
        rem_mass_array[high_metal_low_mass_idx] = self.M_rem_high_metal_low_mass(Z_array[high_metal_low_mass_idx],
                                                                                 core_mass[high_metal_low_mass_idx])
        ...
        rem_mass_array[high_metal_med_mass_idx] = self.M_rem_high_metal_med_mass(Z_array[high_metal_med_mass_idx],
                                                                                 core_mass[high_metal_med_mass_idx])

        # remnant masses of stars with Z > 4.0e-3 and MZAMS > 10.0
        high_metal_high_mass_idx = np.where((Z_array > 4.0e-3) & (core_mass > 10.0))
        rem_mass_array[high_metal_high_mass_idx] = self.M_rem_high_metal_high_mass(Z_array[high_metal_high_mass_idx],
                                                                                   core_mass[high_metal_high_mass_idx])
