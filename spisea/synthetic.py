        ...
        if self.ifmr != None:
            ...
            if 'metallicity_array' in inspect.getfullargspec(self.ifmr.generate_death_mass).args:
                r_mass_tmp, r_id_tmp = self.ifmr.generate_death_mass(mass_array=star_systems['mass'][idx_rem],
                                                                     metallicity_array=star_systems['metallicity'][
                                                                         idx_rem])
            else:
                r_mass_tmp, r_id_tmp = self.ifmr.generate_death_mass(mass_array=star_systems['mass'][idx_rem])
    ...
    def _make_companions_table(self, star_systems, compMass):
        ...
        if isinstance(self.imf._multi_props, multiplicity.MultiplicityResolvedDK):
            ...
            for ii in range(len(companions)):
                companions['log_a'][ii] = self.imf._multi_props.log_semimajoraxis(
                    star_systems['mass'][companions['system_idx'][ii]])

            companions['e'] = self.imf._multi_props.random_e(np.random.rand(N_comp_tot))
            companions['i'], companions['Omega'], companions[
                'omega'] = self.imf._multi_props.random_keplarian_parameters(np.random.rand(N_comp_tot),
...
class IsochronePhot(Isochrone):
    ...
    def __init__(self, logAge, AKs, distance,
        ...
        if metallicity == 0.0:
            ...
            self.save_file = save_file_fmt.format(iso_dir, logAge, AKs, str(distance).zfill(5), metal_pre,
                                                  str(metal_flag).zfill(2))
            self.save_file_legacy = save_file_fmt.format(iso_dir, logAge, AKs, str(distance).zfill(5), metal_pre,
                                                         str(metal_flag).zfill(2))
...
def get_filter_info(name, vega=vega, rebin=True):
    ...
    if name.startswith('nirc2'):
        ...
        try:
            ...
            raise Exception(
                'Filter {0} not understood. Check spelling and make sure cdbs/mtab and cdbs/comp files are up to date'.format(
                    name))
